...
class MainCarouselController extends Controller
{
  ...
  {
    $this->middleware('permission:main.create')->only(['create', 'store']);
    $this->middleware('permission:main.index')->only('index');
    $this->middleware('permission:main.edit')->only(['edit', 'update']);
  }

  public function index(Request $request)
  {
    $home  = MainCarousel::where('active_item', '=', 1)->get();
